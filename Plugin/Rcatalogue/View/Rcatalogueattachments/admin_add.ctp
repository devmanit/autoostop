...
$this->Html
	...
	->addCrumb(__d('croogo', 'Rcatalogueattachments'), array('plugin' => 'rcatalogue', 'controller' => 'rcatalogueattachments', 'action' => 'index'))
	->addCrumb(__d('croogo', 'Upload'), '/' . $this->request->url);


$formUrl = array('controller' => 'rcatalogueattachments', 'action' => 'add',$this->request->data['edit']);
if (isset($this->request->params['named']['editor'])) {
	$formUrl['editor'] = 1;
}
echo $this->Form->create('Rcatalogueattachment', array('url' => $formUrl, 'type' => 'file'));
