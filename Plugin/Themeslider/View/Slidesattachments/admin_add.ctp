...
$this->Html
	...
	->addCrumb(__d('croogo', 'Slidesattachments'), array('plugin' => 'themeslider', 'controller' => 'slidesattachments', 'action' => 'index'))
	->addCrumb(__d('croogo', 'Upload'), '/' . $this->request->url);

$formUrl = array('controller' => 'slidesattachments', 'action' => 'add');
if (isset($this->request->params['named']['editor'])) {
	$formUrl['editor'] = 1;
}
echo $this->Form->create('Slidesattachment', array('url' => $formUrl, 'type' => 'file'));
